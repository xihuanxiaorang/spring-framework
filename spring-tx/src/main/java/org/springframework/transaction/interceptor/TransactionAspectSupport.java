...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		// 获取 TransactionAttributeSource，这个对象在初始化 TransactionInterceptor 时就已经初始化了
		TransactionAttributeSource tas = getTransactionAttributeSource();
		// 从 TransactionAttributeSource 中获取事务属性
		final TransactionAttribute txAttr = (tas != null ? tas.getTransactionAttribute(method, targetClass) : null);
		// 确定并返回事务管理器，容器中不存在就创建
		...
		if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager)) {
			...
			/*
			创建事务信息对象(TransactionInfo)，包含如下属性
			 - 事务管理器(DataSourceTransactionManager)
			 - 事务属性(TransactionAttribute)
			 - 事务状态(TransactionStatus)，包含如下属性：
			   - 事务(transaction)
			   - 是否新事务(newTransaction)
			   - 挂起的资源(suspendedResources)
			 - 老的事务信息对象(TransactionInfo)
			 */
	...
	protected TransactionInfo prepareTransactionInfo(@Nullable PlatformTransactionManager tm,
		...
		// 绑定到当前线程，又是一次 ThreadLocal 的操作(将原来的TransactionInfo放到oldTransactionInfo中保存，将新的TransactionInfo绑定到当前线程)
		txInfo.bindToThread();
		return txInfo;
	}
	...
	protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
		...
		if (txInfo != null && txInfo.getTransactionStatus() != null) {
			...
			// 判断抛出的异常是否满足回滚条件，默认情况下只对 RuntimeException 和 Error 类型的异常进行回滚
