...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		synchronized (mbd.postProcessingLock) {
			if (!mbd.postProcessed) {
				try {
					// 循环遍历 MergedBeanDefinitionPostProcessor 后置处理器接口的实现类，调用实现类中的 postProcessMergedBeanDefinition 方法
					// 允许对指定 bean 的给定合并 bean 定义信息进行后处理
		...
		if (earlySingletonExposure) {
			...
			// getEarlyBeanReference => 获取原始对象的早期引用，其中会执行 AOP 相关逻辑。若 bean 未被 AOP 拦截，getEarlyBeanReference 原样返回 bean。
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// 静态工厂或者实例工厂调用factoryMethod方法创建bean实例
		...
		// 在此处循环遍历SmartInstantiationAwareBeanPostProcessor接口的实现类，调用实现类中的determineCandidateConstructors方法，指定使用哪个构造器方法
	...
	protected BeanWrapper instantiateBean(String beanName, RootBeanDefinition mbd) {
		try {
			...
			if (System.getSecurityManager() != null) {
				...
				// 使用实例化策略创建bean实例(利用反射技术使用构造方法创建bean实例)
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// 循环遍历InstantiationAwareBeanPostProcessor接口的实现类，执行实现类中的postProcessAfterInstantiation方法
		// 如果方法返回false，那么就不再执行后续的流程直接返回；如果返回true，则继续往下走
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
		...
		if (hasInstAwareBpps) {
			...
			// 循环遍历 InstantiationAwareBeanPostProcessor 后置处理器接口的实现类，调用实现类中的 postProcessProperties 方法
			// 自动装配功能  -> 当注解模式下，有一个关键的实现类 AutowiredAnnotationBeanPostProcessor 后置处理器，使用自动注入的方式给属性赋值
			for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		// 获取属性值列表，如果pvs是使用 MutablePropertyValues 封装的类型，则直接强转
		if (pvs instanceof MutablePropertyValues) {
			mpvs = (MutablePropertyValues) pvs;
			if (mpvs.isConverted()) {
				// Shortcut: use the pre-converted values as-is.
				// 如果mpvs中的值已经被转换成对应的类型，那么可以直接设置到BeanWrapper中
				try {
		...
		// 如果存在自定义的TypeConverter，则使用自定义的，否则直接使用入参的bw
		TypeConverter converter = getCustomTypeConverter();
		if (converter == null) {
			converter = bw;
		}
		// 获取对应的解析器（为给定的 BeanFactory 和 BeanDefinition 创建一个 BeanDefinitionValueResolver）
		...
		for (PropertyValue pv : original) {
			if (pv.isConverted()) {
				// 如果pv已经是转换过，则直接添加到 deepCopy 集合中
				deepCopy.add(pv);
			}
			else {
				// 获取属性名
				...
				// 通过解析器 BeanDefinitionValueResolver 解析原始属性值（点进去）
	...
	protected Object initializeBean(String beanName, Object bean, @Nullable RootBeanDefinition mbd) {
		if (System.getSecurityManager() != null) {
			...
			// 执行Aware接口中的回调方法(beanName \ classLoader \ beanFactory)
			invokeAwareMethods(beanName, bean);
		}

		Object wrappedBean = bean;
		if (mbd == null || !mbd.isSynthetic()) {
			// 循环遍历 BeanPostProcessor 接口的实现类，调用实现类中的 postProcessBeforeInitialization 方法
			// 其中有一个关键的实现类 ApplicationContextAwareProcessor 后置处理器，在此后置处理器中的此方法中会执行bean实现Aware接口时重写的回调方法
